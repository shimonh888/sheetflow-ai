...
export default function MultiFilePreviewPage() {
    ...
    const [files, setFiles] = useState<FileSourceCreate[]>([])
    ...
    useEffect(() => {
        ...
        // Get selected files from session storage
        const storedFiles = sessionStorage.getItem('sheetflow_selected_files')
        const storedDescription = sessionStorage.getItem('sheetflow_global_description')
        ...
        const parsedFiles = JSON.parse(storedFiles) as FileSourceCreate[]
        setFiles(parsedFiles)
        setGlobalDescription(storedDescription || '')

        // Set default title from first file
        if (parsedFiles.length > 0) {
            const firstName = parsedFiles[0].file_name.replace(/\.[^/.]+$/, '') // Remove extension
            setDashboardTitle(parsedFiles.length > 1
                ? `${firstName} + ${parsedFiles.length - 1} more`
                : firstName)
        }

        loadPreview(parsedFiles, storedDescription || '')
    }, [router])

    const loadPreview = async (files: FileSourceCreate[], description: string) => {
        setIsLoading(true)
        setError(null)

        try {
            const data = await previewDashboard(files, description)
            setPreviewData(data)
    ...
    const handleCreateDashboard = async () => {
        if (!previewData || files.length === 0) return
        ...
        try {
            ...
            sessionStorage.removeItem('sheetflow_selected_files')
            sessionStorage.removeItem('sheetflow_global_description')
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-dark-900">
                <div className="text-center">
                    <LoadingSpinner size="lg" message="Analyzing your files..." />
                    <p className="text-dark-400 mt-4 text-sm">
                        The AI is reviewing {files.length} file{files.length !== 1 ? 's' : ''} to suggest the best visualizations
                    </p>
                </div>
            </div>
        )
            ...
            <div className="min-h-screen flex items-center justify-center bg-dark-900">
                <div className="text-center max-w-md">
                    ...
                    <div className="flex gap-3 justify-center">
                        ...
                        <button
                            onClick={() => loadPreview(files, globalDescription)}
    ...
    return (
        <div className="min-h-screen bg-dark-900">
            {/* Header */}
            <header className="border-b border-white/10 bg-dark-900/80 backdrop-blur-sm sticky top-0 z-10">
            ...
            <main className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="mb-6 flex flex-wrap gap-2">
                    {files.map((file, index) => (
                        <div
                            key={index}
                            className="inline-flex items-center gap-2 px-3 py-1.5 bg-dark-800 rounded-lg border border-white/10"
                        >
                            <FileSpreadsheet className="w-4 h-4 text-primary-400" />
                            <span className="text-sm text-white">{file.file_name}</span>
                            {file.file_context && (
                                <span className="text-xs text-dark-400 italic truncate max-w-32">
                                    • {file.file_context}
                                </span>
                            )}
                        </div>
                ...
                {/* Info Banner */}
                <div className="bg-primary-400/10 border border-primary-400/20 rounded-xl p-4 mb-8 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-dark-400 text-sm mt-1">
                            Based on your data and context, we've suggested {proposals.length} chart{proposals.length !== 1 ? 's' : ''}.
                ...
                {previewData?.data_summary && (
                    <div className="mb-8 flex gap-4 overflow-x-auto pb-2">
                        <div className="bg-dark-800 rounded-lg px-4 py-2 flex-shrink-0">
                            <p className="text-dark-400 text-xs">Rows</p>
                            <p className="text-white font-semibold">{previewData.data_summary.row_count}</p>
                        </div>
                        <div className="bg-dark-800 rounded-lg px-4 py-2 flex-shrink-0">
                            <p className="text-dark-400 text-xs">Columns</p>
                            <p className="text-white font-semibold">{previewData.data_summary.columns.length}</p>
                        </div>
                        <div className="bg-dark-800 rounded-lg px-4 py-2 flex-shrink-0">
                            <p className="text-dark-400 text-xs">Sheets</p>
                            <p className="text-white font-semibold">{previewData.sheet_names.length}</p>
                        </div>
                        <div className="bg-dark-800 rounded-lg px-4 py-2 flex-shrink-0">
                            <p className="text-dark-400 text-xs">Files</p>
                ...
                {proposals.length > 0 ? (
                                ...
                                >
                                    ...
                                    <div className="p-5 border-b border-white/5">
                                        <div className="flex items-start gap-4">
                                            <div className="w-12 h-12 rounded-lg bg-primary-400/10 flex items-center justify-center flex-shrink-0">
                                                <ChartIcon className="w-6 h-6 text-primary-400" />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-transparent text-white font-semibold text-lg border-b border-transparent hover:border-white/20 focus:border-primary-400 focus:outline-none transition-colors pb-1"
                                                ...
                                                <div className="mt-2 relative">
                                                    <select
                                                        ...
                                                        className="w-full appearance-none bg-white/5 text-dark-300 text-sm rounded-lg px-3 py-2 pr-8 border border-white/10 focus:border-primary-400 focus:outline-none cursor-pointer"
                                                    >
                                    ...
                                    <div className="px-5 py-3 bg-white/[0.02] flex gap-6 text-sm">
                                        {proposal.x_axis && (
                                            <div>
                                                <span className="text-dark-500">X-Axis:</span>
                                                <span className="text-dark-300 ml-2">{proposal.x_axis}</span>
                                            </div>
                                        )}
                                        {proposal.y_axis && (
                                            <div>
                                                <span className="text-dark-500">Y-Axis:</span>
                                                <span className="text-dark-300 ml-2">{proposal.y_axis}</span>
                                            </div>
                                        )}
                                        {proposal.data_key && !proposal.y_axis && (
                                            <div>
                                                <span className="text-dark-500">Data:</span>
                                                <span className="text-dark-300 ml-2">{proposal.data_key}</span>
                                            </div>
                                        )}
                                    </div>
                    ...
                    <div className="text-center py-12 bg-dark-800 rounded-xl">
                        ...
                        <p className="text-dark-400">No chart suggestions available</p>
                        <p className="text-dark-500 text-sm mt-1">The AI couldn't determine suitable charts for this data</p>
                    </div>
                )}
            </main>
        </div>
